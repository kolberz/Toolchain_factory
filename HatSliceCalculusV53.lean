...
theorem mem_succPairs (U : Finset α) (j : ℕ) (p : Finset α × α) :
  ...
  · intro hp
    ...
    exact ⟨hT, hpMem⟩
  · rintro ⟨hT, hv⟩
    have hTU : p.1 ⊆ U := (Finset.mem_powersetCard.mp hT).1
...
theorem sum_insertion_pairs_eq_slice_sum
  ...
  calc
    (∑ S ∈ deletionSlice U j, ∑ v ∈ U \ S, L (insert v S))
        = ∑ p ∈ insertionPairs U j, L (insert p.2 p.1) := h_left.symm
    _ = ∑ q ∈ succPairs U j, L q.1 := h_flat
    _ = ∑ T ∈ deletionSlice U (j + 1), ∑ v ∈ T, L T := h_right
    _ = (j + 1 : ℚ) * (∑ T ∈ deletionSlice U (j + 1), L T) := by
      calc
        (∑ T ∈ deletionSlice U (j + 1), ∑ _ ∈ T, L T)
            = ∑ T ∈ deletionSlice U (j + 1), ((T.card : ℚ) * L T) := by
                apply Finset.sum_congr rfl
                intro T hT
                rw [Finset.sum_const, nsmul_eq_mul]
        _ = ∑ T ∈ deletionSlice U (j + 1), ((j + 1 : ℚ) * L T) := by
              apply Finset.sum_congr rfl
              intro T hT
...
theorem sum_sdiff_const_loss
    ...
    (∑ S ∈ deletionSlice U j, ∑ _ ∈ U \ S, L S) =
      (((U.card - j : ℕ) : ℚ) * (∑ S ∈ deletionSlice U j, L S)) := by
  have h_sdiff : ∀ S ∈ deletionSlice U j, (U \ S).card = U.card - j := by
    intro S hS
    have hSU : S ⊆ U := (Finset.mem_powersetCard.mp hS).1
    have hcard : S.card = j := (Finset.mem_powersetCard.mp hS).2
    rw [Finset.card_sdiff_of_subset hSU, hcard]
  calc
    (∑ S ∈ deletionSlice U j, ∑ _ ∈ U \ S, L S)
        = ∑ S ∈ deletionSlice U j, (((U \ S).card : ℚ) * L S) := by
            apply Finset.sum_congr rfl
            intro S hS
            rw [Finset.sum_const, nsmul_eq_mul]
    _ = ∑ S ∈ deletionSlice U j, (((U.card - j : ℕ) : ℚ) * L S) := by
          apply Finset.sum_congr rfl
          intro S hS
          rw [h_sdiff S hS]
    _ = ((U.card - j : ℕ) : ℚ) * (∑ S ∈ deletionSlice U j, L S) := by
          rw [Finset.mul_sum]
...
theorem slice_average_diff_eq_pair_average
    (U : Finset α) (L : Finset α → ℚ) (j : ℕ) (hj : j + 1 ≤ U.card) :
    let A (k : ℕ) := (∑ S ∈ deletionSlice U k, L S) / ((deletionSlice U k).card : ℚ)
    A (j + 1) - A j =
  ...
  have h_deriv :
      (∑ S ∈ deletionSlice U j, ∑ v ∈ U \ S, discreteDeriv L S v) =
        (∑ S ∈ deletionSlice U j, ∑ v ∈ U \ S, L (insert v S)) -
        (∑ S ∈ deletionSlice U j, ∑ v ∈ U \ S, L S) := by
  ...
  let k : ℚ := (j + 1 : ℚ)
  let s1 : ℚ := ∑ S ∈ deletionSlice U j, L S
  let s2 : ℚ := ∑ T ∈ deletionSlice U (j + 1), L T
